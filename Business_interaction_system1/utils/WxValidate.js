...
class WxValidate {
    ...
    __initDefaults() {
        this.defaults = {
            messages: {
              ...
              checkNacust_uniformSCcodeme: '请输入统一社会信用代码',///[^_IOZSVa-z\W]{2}\d{6}[^_IOZSVa-z\W]{10}/
              checkCust_name: '请输入公司全称',///^[\u4E00-\u9FA5A-Za-z0-9]+$/
              checkCust_linkman: '请输入联系人姓名或称呼',///^[\u4E00-\u9FA5A-Za-z\s]+(·[\u4E00-\u9FA5A-Za-z]+)*$/
              checkCust_linkman_role: '请输入职位信息',///^[\u4e00-\u9fa5]{0,}$/
              checkCust_phone_num: '请输入11位纯数字',///^(13[0-9]|14[5|7]|15[0|1|2|3|5|6|7|8|9]|18[0|1|2|3|5|6|7|8|9])\d{8}$/
              checkCust_address: '请输入客户地址',///([^\x00-\xff]|[A-Za-z0-9_])+/
              checkBos_name: "请输入商机名称",///([^\x00-\xff]|[A-Za-z0-9_])+/
              checkDouble: "请输入数字，小数点",///^(:?(:?\d+.\d+)|(:?\d+))$/
              checkDataTime: "格式为：2019-1-1",///([0-9]{3}[1-9]|[0-9]{2}[1-9][0-9]{1}|[0-9]{1}[1-9][0-9]{2}|[1-9][0-9]{3})-(((0[13578]|1[02])-(0[1-9]|[12][0-9]|3[01]))|((0[469]|11)-(0[1-9]|[12][0-9]|30))|(02-(0[1-9]|[1][0-9]|2[0-8])))/
            }
        }
    }
    ...
    __initMethods() {
        ...
        that.methods = {
             ...
             * 验证统一社会信用代码
             */
            checkNacust_uniformSCcodeme(value) {
              return that.optional(value) || /[^_IOZSVa-z\W]{2}\d{6}[^_IOZSVa-z\W]{10}/.test(value)
            },
            /**
             * 验证公司名称
             */
            checkCust_name(value) {
              return that.optional(value) || /^[\u4E00-\u9FA5A-Za-z0-9]+$/.test(value)
            },
            /**
             * 验证客户联系人
             */
            checkCust_linkman(value) {
              return that.optional(value) || /^[\u4E00-\u9FA5A-Za-z\s]+(·[\u4E00-\u9FA5A-Za-z]+)*$/.test(value)
            },
              /**
             * 验证联系人职位
             */
          checkCust_linkman_role(value) {
            return that.optional(value) || /^[\u4e00-\u9fa5]{0,}$/.test(value)
            },
          /**
           * 验证联系电话
           */
          checkCust_phone_num(value) {
            return that.optional(value) || /^(13[0-9]|14[5|7]|15[0|1|2|3|5|6|7|8|9]|18[0|1|2|3|5|6|7|8|9])\d{8}$/.test(value)
          },
          /**
           * 验证详细地址
           */
          checkCust_address(value) {
            return that.optional(value) || /([^\x00-\xff]|[A-Za-z0-9_])+/.test(value)
          },
          checkBos_name(value) {
            return that.optional(value) || /([^\x00-\xff]|[A-Za-z0-9_])+/.test(value)
          },
          checkDouble(value){
            return that.optional(value) || /^(:?(:?\d+.\d+)|(:?\d+))$/.test(value)
          },
          checkDataTime(value) {
            return that.optional(value) || /([1-9]\d{3})-(\d{1}|\d{2})-(\d{1}|\d{2})/.test(value)
          },
            /**
             * 验证电子邮箱格式
             */
